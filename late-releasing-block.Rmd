...
new_block_conversion <- blocks_combined_type %>%
  pivot_wider(names_from = c("new_block","release_block"),
              values_from = total,
              values_fill = 0) %>%
  mutate(incorrect_new_block = ifelse(NON_NEW_BLOCK_NON_TIME_RELEASE>0 & NEW_BLOCK_NON_TIME_RELEASE>0, 1, 0),
...
order <- c("Site","Specialty","Department","DEPARTMENT_ID","Provider","Slot.YearMonth",
...
# # Slot Length, Unavailability Information 
# block_raw_test <- block_raw_tbl %>%
#   head(10) %>%
#   collect()
# 
# availability_raw_test <- availability_raw_tbl %>%
#   head(10) %>%
#   collect()
# 
# access_raw_test <- access_raw_tbl %>%
#   head(10) %>%
#   collect()
# 
# 
# block_raw_test <- block_raw_tbl %>%
#   left_join(availability_tbl %>% dplyr::select(APPT_BLOCK_C, NAME), 
#             by = c("BLOCK_C" = "APPT_BLOCK_C")) %>%
#   
# 
# 
...
#   filter(Slot.Year == 2023) %>%
#   filter(Slot.Month >= 04) %>%
#   dplyr::select(DEPARTMENT_ID, PROV_ID, SLOT_BEGIN_TIME, NUM_APTS_SCHEDULED, ORG_REG_OPENINGS, ORG_OVBK_OPENINGS,
#                 SLOT_LENGTH, UNAVAILABLE_RSN_NAME, PRIVATE_YN, APPT_OVERBOOK_YN, APPT_BLOCK_NAME,
#                 APPT_PRC_NAME, PAT_ENC_CSN_ID, DAY_UNAVAIL_RSN_NAME, 
#                 TIME_UNAVAIL_RSN_NAME, DAY_HELD_RSN_NAME, TIME_HELD_RSN_NAME, OUTSIDE_TEMPLATE_YN) %>%
#   collect()
# 
# 
...
#   filter(Appt.Month >= 04) %>%
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(DEPARTMENT_ID, PROV_ID, PROV_NAME_WID, VISIT_PROV_STAFF_RESOURCE_C,
#            PRC_NAME, APPT_LENGTH, VISIT_TYPE, VISIT_METHOD, VISIT_GROUP_NUM) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   dplyr::select(-total)
# 
# 
# appt_type_mapping <- access_raw %>%
#   group_by(DEPARTMENT_ID, PROV_ID, PROV_NAME_WID, VISIT_PROV_STAFF_RESOURCE_C,
#            PRC_NAME, VISIT_TYPE, VISIT_METHOD, VISIT_GROUP_NUM) %>%
#   summarise(total = n()) %>%
#   dplyr::select(-total)
#   
# 
# 
# 
# booked_appts <- availability_raw %>%
#   group_by(DEPARTMENT_ID, PROV_ID, SLOT_BEGIN_TIME, SLOT_LENGTH, UNAVAILABLE_RSN_NAME, APPT_BLOCK_NAME, APPT_PRC_NAME, PAT_ENC_CSN_ID) %>%
#   summarise(total = n()) %>%
#   group_by(DEPARTMENT_ID, PROV_ID, SLOT_BEGIN_TIME, SLOT_LENGTH, UNAVAILABLE_RSN_NAME, APPT_BLOCK_NAME, APPT_PRC_NAME) %>%
#   summarise(total_appts = n())
# 
# 
# booked_appts <- left_join(booked_appts, appt_type_mapping, by = c("DEPARTMENT_ID" = "DEPARTMENT_ID",
#                                                            "PROV_ID" = "PROV_ID",
#                                                            "APPT_PRC_NAME" = "PRC_NAME"))
# 
# 
# 
# 
...
# 
# 
# block_appt_dur_comparison <- blocks_combined %>%
#   group_by(Campus, DEPARTMENT_ID, Department, Provider, Slot.Year, Slot.Month, Slot.YearMonth, Slot.DateYear, SLOT_BEGIN_TIME, block_combination) %>%
#   summarise(total = n()) 
# 
# block_appt_dur_comparison <- block_appt_dur_comparison %>%
#   group_by(DEPARTMENT_ID, Provider, Slot.DateYear) %>%
#   mutate(previous_slot_time = lag(SLOT_BEGIN_TIME)) %>%
#   mutate(block_length = difftime(SLOT_BEGIN_TIME, previous_slot_time, "mins"))
# 
# block_appt_dur_comparison <- block_appt_dur_comparison %>%
#   mutate(block_length = as.numeric(block_length)/60)
# 
# block_appt_dur_comparison_summary <- block_appt_dur_comparison %>%
#   filter(!is.na(block_length)) %>%
#   group_by(Campus, DEPARTMENT_ID, Department, Provider, Slot.YearMonth, block_combination, block_length) %>%
#   summarise(total_blocks = n()) 
# 
# 
# test <- block_appt_dur_comparison_summary %>% 
#   group_by(Campus, Department, Provider, block_length) %>%
#   summarise(total = sum(total_blocks))
#   
...
reactable(
  ...
  groupBy = c("Site", "Department", "Provider", "Slot.YearMonth"),
  ...
  columns = list(
    ...
    correct_new_block = colDef(
      name = "Clean Non-Late-Releasing Blocks",
      maxWidth = 100,
    ...
    incorrect_new_block = colDef(
      name = "Unclean Non-Late-Releasing Combo Blocks",
      maxWidth = 100,
      headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = 'sum',
      format = colFormat(separators = TRUE, digits = 0),
    ...
    correct_time_release_block = colDef(
      name = "Clean Late-Releasing Blocks",
      maxWidth = 100,
    ...
    incorrect_time_release_block = colDef(
      name = "Unclean Late-Releasing Combo Blocks",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = 'sum',
      format = colFormat(separators = TRUE, digits = 0),
