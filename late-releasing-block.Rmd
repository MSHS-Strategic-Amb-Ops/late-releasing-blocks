...
#   mutate(wait_time = difftime(APPT_DTTM - APPT_MADE_DTTM, "days")) %>%
#   filter(wait_time >= 0) %>%
#   group_by(DEPARTMENT_ID, DEPARTMENT_NAME, PROV_ID, PROV_NAME_WID, Appt.Year, Appt.Month) %>%
#   summarise(total = n(),
#             new = sum(VISIT_GROUP_NUM == 4),
#             new_14day = sum(VISIT_GROUP_NUM == 4 & wait_time <=14),
#             med_wait_time = median(wait_time, na.rm = TRUE)) %>%
#   mutate(new_perc = round(new/total),
#          new_14day_perc = round(new_14day/new)) %>%
#   show_query()
#   collect()
#   
#   
...
#   left_join(access_raw_tbl %>% dplyr::select(PAT_ENC_CSN_ID, PRC_NAME, APPT_LENGTH, VISIT_GROUP_NUM, APPT.MADE),
#             by = c("PAT_ENC_CSN_ID")) %>%
#   collect()
...
#            SLOT_LENGTH, PAT_ENC_CSN_ID, APPT_PRC_NAME, APPT_LENGTH, VISIT_GROUP_NUM) %>%
#   summarise(total_blocks = n()) %>%
#   group_by(DEPARTMENT_ID, PROV_ID, Slot.DateYear, block_type, comb_block_name, SLOT_LENGTH, APPT_PRC_NAME, APPT_LENGTH, VISIT_GROUP_NUM) %>%
#   summarise(total_blocks = n()) %>%
#   collect()
# 
# block_raw_test <- block_raw_test %>%
#   mutate(incorrect = ifelse(APPT_LENGTH>=SLOT_LENGTH, APPT_LENGTH  %% SLOT_LENGTH, SLOT_LENGTH %% APPT_LENGTH))
# 
# 
# block_raw_test$Site <- dept_mapping$SITE[match(block_raw_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test$Specialty <- dept_mapping$DEPT_SPECIALTY_NAME[match(block_raw_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test$Department <- dept_mapping$DEPARTMENT_NAME[match(block_raw_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test$Provider <- prov_mapping$PROV_NAME[match(block_raw_test$PROV_ID, prov_mapping$PROV_ID)]
# 
