...
blocks_combined <- late_block_raw %>%
  ...
  ungroup() %>%
  arrange(DEPARTMENT_ID, SLOT_BEGIN_TIME, comb_block_name) %>%
  group_by(DEPARTMENT_ID, PROV_ID, ACTIVE_STATUS, SLOT_BEGIN_TIME, Slot.DateYear, LINE, comb_block_name, ORG_AVAIL_BLOCKS, ORG_REG_OPENINGS) %>%
  summarise(total_blocks = n())
...
block_openings <- blocks_combined %>%
  ...
  mutate(block_combination = paste0(unique(comb_block_name), collapse = ', ')) %>%
  group_by(DEPARTMENT_ID, PROV_ID, ACTIVE_STATUS, SLOT_BEGIN_TIME, Slot.DateYear, ORG_REG_OPENINGS, block_combination, block_type) %>%
...
blocks_combined_type <- blocks_combined %>%
  ...
  mutate(block_combination = paste0(unique(comb_block_name), collapse = ', ')) %>%
  group_by(DEPARTMENT_ID, PROV_ID, PROV_TYPE, ACTIVE_STATUS, SLOT_BEGIN_TIME,  block_combination, new_block, release_block) %>%
  summarise(total = n()) %>%
  mutate(Slot.YearMonth = format(as.Date(SLOT_BEGIN_TIME),"%Y-%m"))
...
new_block_conversion <- blocks_combined_type %>%
  ...
  group_by(Site, Specialty, Department, DEPARTMENT_ID, Provider, PROV_TYPE, ACTIVE_STATUS, Slot.YearMonth) %>%
...
order <- c("Site","Department","Specialty","DEPARTMENT_ID","Provider","PROV_TYPE", "ACTIVE_STATUS", "Slot.YearMonth",
