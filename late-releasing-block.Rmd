...
```{r Late-Releasing Wait Time, echo = FALSE, warning = FALSE, message = FALSE}

# # Look at % of late-releasing conversion, No Show and Wait Times
# ## Wait Time
# wait_time <- access_raw_tbl %>%
...
#          new.14days = ifelse(wait_time <= 14, "Yes", "No")) %>%
#   group_by(DEPARTMENT_ID, Appt.Year, Appt.Month, New.PT2) %>%
#   mutate(med_wait_time_dept = median(wait_time, na.rm = TRUE)) %>%
#   group_by(DEPARTMENT_ID, DEPARTMENT_NAME, med_wait_time_dept, PROV_ID, Appt.Year, Appt.Month, New.PT2) %>%
#   mutate(med_wait_time_prov = median(wait_time, na.rm = TRUE)) %>%
#   group_by(DEPARTMENT_ID, DEPARTMENT_NAME, PROV_ID, Appt.Year, Appt.Month, med_wait_time_dept, med_wait_time_prov, New.PT2, new.14days) %>%
#   summarise(total = n()) %>%
#   collect()
# 
# wait_time <- wait_time %>%
#   group_by(DEPARTMENT_ID, Appt.Year, Appt.Month, new.14days) %>%
#   mutate(new_14day_perc_dept = round(total/sum(total),2)) %>%
#   ungroup() %>%
#   group_by(DEPARTMENT_ID, Appt.Year, Appt.Month) %>%
#   mutate(new_perc_dept = round(total/sum(total),2)) %>%
#   ungroup() %>%
#   group_by(DEPARTMENT_ID, PROV_ID, Appt.Year, Appt.Month, new.14days) %>%
#   mutate(new_14day_perc_prov = round(total/sum(total),2)) %>%
#   ungroup() %>%
#   group_by(DEPARTMENT_ID, PROV_ID, Appt.Year, Appt.Month) %>%
#   mutate(new_perc_provider = round(total/sum(total),2))
...
# block_raw_test_test <- late_block_raw %>%
...
#   left_join(access_raw_tbl %>% dplyr::select(PAT_ENC_CSN_ID, PRC_NAME, APPT_LENGTH, VISIT_GROUP_NUM, APPT_MADE_DATE),
#             by = c("PAT_ENC_CSN_ID")) %>%
...
#            SLOT_LENGTH, PAT_ENC_CSN_ID, APPT_BLOCK_NAME, APPT_PRC_NAME, APPT_LENGTH, VISIT_GROUP_NUM) %>%
#   summarise(total_blocks = n()) %>%
...
#   collect()
# 
# block_raw_test_test <- block_raw_test_test %>%
#   mutate(incorrect = ifelse(APPT_LENGTH>=SLOT_LENGTH, APPT_LENGTH  %% SLOT_LENGTH, SLOT_LENGTH %% APPT_LENGTH))
# 
# 
# block_raw_test_test$Site <- dept_mapping$SITE[match(block_raw_test_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test_test$Specialty <- dept_mapping$DEPT_SPECIALTY_NAME[match(block_raw_test_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test_test$Department <- dept_mapping$DEPARTMENT_NAME[match(block_raw_test_test$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
# block_raw_test_test$Provider <- prov_mapping$PROV_NAME[match(block_raw_test_test$PROV_ID, prov_mapping$PROV_ID)]
