...
block_combination_usage <- late_block_raw %>%
  arrange(Campus, DEPARTMENT_ID, Department, SLOT_BEGIN_TIME, block_name, org_block_name) %>%
  ...
  # mutate(block_combination = paste0(unique(block_name[block_name!='']), collapse = ', ')) %>%
  mutate(Slot.Year = year(SLOT_BEGIN_TIME),
         Slot.Month = month(SLOT_BEGIN_TIME)) %>%
  group_by(Campus, DEPARTMENT_ID, Department, Provider, Slot.DateYear, block_combination) %>%
...
block_combination_usage <- block_combination_usage %>%
  mutate(slot_incorrect = case_when(total_blocks >1 & str_detect(block_combination, "New Patient 7-Day|New Patient 14-Day|New Patient 21-Day|New Patient 28-Day") ~ "Yes",
                                    TRUE ~ "No"))
