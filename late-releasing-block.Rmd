...
clarity_ser_myc_tbl <- tbl(conn2, "CLARITY_SER_MYC")
clarity_dep_myc_tbl <- tbl(conn2, "CLARITY_DEP_MYC")
...
slot_view_block_tbl <- tbl(conn3, "AMBULATORY_SLOT_BLOCK_TABLE")

conn4 <- dbPool(drv = odbc(), dsn = "OAO Cloud DB Production", timeout = 30)
slot_view_tbl <- tbl(conn4, in_schema("OAO_PRODUCTION", "AMBULATORY_SLOT_TABLE"))

# block_name <- block_name_tbl %>%
#   collect()
# 
...
#   collect()
# 
# access_arrived <- access %>%
#   filter(DERIVED_STATUS_DESC == "Arrived")
# 
# prc <- access_arrived %>% filter(PRC_ID == 9875) %>% group_by(PRC_ID, PRC_NAME, APPT_BLOCK_C, APPT_BLOCK_NAME) %>% summarise(total = n())
# require(openxlsx)
# write.xlsx(prc, "PRC ID 9875.xlsx")
```
...
#   group_by(appt_year, appt_month, SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID,
#            PROV_ID, PROV_NAME_WID, VISIT_PROV_STAFF_RESOURCE_C,
#            VISIT_METHOD, PRC_NAME, VISIT_TYPE, APPT_LENGTH) %>%
#   summarise(total = n()) %>%
#   collect()
# 
# urology_procedures <- access_raw_tbl %>%
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   mutate(appt_year = year(CONTACT_DATE),
#          appt_month = month(CONTACT_DATE)) %>%
#   filter(appt_year >= 2023) %>% 
#   filter(DEPARTMENT_NAME %like% "=")
...
#   filter(year >= 2022) %>%
#   filter(DERIVED_STATUS_DESC == "Arrived") %>%
#   group_by(year, month, SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID, PROV_ID, APPT_BLOCK_NAME, PRC_NAME, VISIT_GROUP_NUM) %>%
#   summarise(total_arrived_appts = n(),
#             total_arrived_hours = sum(APPT_LENGTH)) %>%
#   collect()
# 
# 
...
# 
# 
# access_raw_block <- access_raw %>%
#   left_join(clarity_ser) %>%
#   mutate(total_arrived_hours = round(total_arrived_hours/60,1)) %>%
#   mutate(NEW_APPT_YN = ifelse(is.na(VISIT_GROUP_NUM), "N", "Y"),
#          NEW_BLOCK_YN = ifelse(APPT_BLOCK_NAME %in% new_blocks, "Y", "N")) %>%
#   ungroup() %>%
#   dplyr::select(year, month, SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME, PROV_ID, PROV_TYPE, 
#                 APPT_BLOCK_NAME, NEW_BLOCK_YN,
#                 PRC_NAME, NEW_APPT_YN,
#                 total_arrived_appts, total_arrived_hours)
# 
# access_raw_block_filtered <- access_raw_block %>%
#   filter(paste0(year,month) != "20247") %>%
#   filter(paste0(year,month) != "20248")
# 
# require(openxlsx)
# write.xlsx(access_raw_block_filtered, "Volume and Hours by Block Type_Jan22_Jun24.xlsx")
# 
# 
# 
# slot_view_block <- slot_view_block_tbl %>%
#   mutate(slot_year = year(SLOT_DATE),
#          slot_month = month(SLOT_DATE)) %>%
#   filter(slot_year == 2023) %>%
#   # filter(SLOT_DATE < as.Date("2024-07-01")) %>%
#   group_by(slot_year, slot_month, SLOT_DATE, CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, PROV_ID, 
#            BLOCK_NAME, ORG_BLOCK_NAME) %>%
#   summarise(AVAILABLE_MINS = sum(AVAILABLE_MINS, na.rm = T)) %>%
#   collect()
# 
# slot_view_block_collapsed <- slot_view_block %>%
#   group_by(slot_year, slot_month, SLOT_DATE, CAMPUS, CAMPUS_SPECIALTY, DEPARTMENT, PROVIDER, PROV_ID) %>%
#   summarise(AVAILABLE_MINS = sum(AVAILABLE_MINS, na.rm = T)) %>%
#   rename("AVAILABLE_MINS_BLOCK" = AVAILABLE_MINS)
# 
# 
# 
...
#   collect()
# 
# slot_view$AVAILABLE_MINS[is.na(slot_view$AVAILABLE_MINS)] <- 0
# 
# slot_merged <- merge(slot_view, slot_view_block_collapsed)
...
late_block_raw <- block_raw_tbl %>%
  ...
  filter(Slot.DateYear >= as.Date(date_start)) %>%
  ...
  left_join(availability_raw_tbl %>% dplyr::select(PROV_ID, DEPARTMENT_ID, SLOT_BEGIN_TIME, SLOT_LENGTH, ORG_REG_OPENINGS) %>%
              group_by(PROV_ID, DEPARTMENT_ID, SLOT_BEGIN_TIME, SLOT_LENGTH, ORG_REG_OPENINGS) %>%
              summarise(total = n()) %>%
              dplyr::select(-total),
...
#   mutate(comb_block_name = trim(ifelse(is.na(org_block_name), block_name, org_block_name))) %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID, PROV_ID, Appt.Year, Appt.MonNum, comb_block_name, Appt.Status) %>%
#   summarise(total = n()) %>%
#   collect() %>%
#   mutate(block_type = case_when(comb_block_name %in% release_blocks ~ "Late-Releasing",
#                                 comb_block_name %in% new_blocks ~ "Non-Late Releasing New",
#                                 TRUE ~ "Other")) %>%
#   group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID, PROV_ID, Appt.Year, Appt.MonNum, block_type, Appt.Status) %>%
#   summarise(total = sum(total)) %>%
#   pivot_wider(names_from = Appt.Status,
#               values_from = total, 
#               values_fill = 0) %>%
#   mutate(no_show_rate = `No Show`/(Arrived + `No Show`),
#          no_show_plus_rate = (`No Show` + `SameDay Canceled`)/(Arrived + `No Show` + `SameDay Canceled`))
# 
# 
# no_show_summary <- no_show %>%
#   dplyr::select(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_NAME_WID, PROV_ID, Appt.Year, Appt.MonNum, block_type, Arrived, `No Show`, `SameDay Canceled`, no_show_rate, no_show_plus_rate) 
...
blocks_combined <- late_block_raw %>%
  ...
  ungroup() %>%
  arrange(DEPARTMENT_ID, SLOT_BEGIN_TIME, comb_block_name) %>%
  group_by(DEPARTMENT_ID, PROV_ID, ACTIVE_STATUS, SLOT_BEGIN_TIME, SLOT_LENGTH, Slot.DateYear, LINE, comb_block_name, ORG_AVAIL_BLOCKS, ORG_REG_OPENINGS) %>%
  summarise(total_blocks = n())
...
block_openings <- blocks_combined %>%
  mutate(block_type = case_when(comb_block_name %in% release_blocks ~ "release",
                                ...
                                TRUE ~ "non-new")) %>%
  collect() %>%
  group_by(DEPARTMENT_ID, PROV_ID, ACTIVE_STATUS, SLOT_BEGIN_TIME) %>%
  mutate(block_combination = paste0(unique(comb_block_name), collapse = ', ')) %>%
  group_by(DEPARTMENT_ID, PROV_ID, ACTIVE_STATUS, SLOT_BEGIN_TIME, SLOT_LENGTH, Slot.DateYear, ORG_REG_OPENINGS, block_combination, block_type) %>%
...
blocks_combined_type <- blocks_combined %>%
  ...
  group_by(DEPARTMENT_ID, PROV_ID, PROV_TYPE, ACTIVE_STATUS, SLOT_BEGIN_TIME) %>%
  mutate(block_combination = paste0(unique(comb_block_name), collapse = ', ')) %>%
  group_by(DEPARTMENT_ID, PROV_ID, PROV_TYPE, ACTIVE_STATUS, SLOT_BEGIN_TIME, SLOT_LENGTH, block_combination, new_block, release_block) %>%
  summarise(total = n()) %>%
  mutate(Slot.YearMonth = format(as.Date(SLOT_BEGIN_TIME),"%Y-%m"))
...
new_block_conversion <- blocks_combined_type %>%
  ...
  group_by(Site, Specialty, Department, DEPARTMENT_ID, PROV_ID, Provider, PROV_TYPE, ACTIVE_STATUS, Slot.YearMonth, SLOT_LENGTH) %>%
...
order <- c("Site","Department","Specialty","DEPARTMENT_ID", "PROV_ID", "Provider","PROV_TYPE", "ACTIVE_STATUS", "Slot.YearMonth", "SLOT_LENGTH",
...
```{r New Patients Scheduled within 14 Days, echo = FALSE, warning = FALSE, message = FALSE}

waitTime_date_end <- paste0(format(Sys.Date(), "%Y-%m"),"-01")
waitTime_date_start <- as.Date(waitTime_date_end) - months(3)

new_pt_waitTime <- access_raw_tbl %>%
  left_join(clarity_ser_tbl %>% 
              dplyr::select(PROV_ID, PROV_TYPE, ACTIVE_STATUS)) %>%
  mutate(Appt.Made.DateYear = TO_DATE(CONTACT_DATE),
         Appt.Made.Year = year(CONTACT_DATE),
         Appt.Made.Month = month(CONTACT_DATE),
         Appt.Made.YearMonth = paste0(Appt.Made.Year,"-",Appt.Made.Month)) %>%
  filter(Appt.Made.DateYear >= as.Date(waitTime_date_start)) %>%
  filter(Appt.Made.DateYear < as.Date(waitTime_date_end)) %>%
  mutate(wait_time = CONTACT_DATE-APPT_MADE_DATE) %>%
  # filter(wait_time >= 0) %>%
  filter(VISIT_GROUP_NUM == 4) %>%
  mutate(waitTime_14days = ifelse(wait_time <= 14, "Yes", "No")) %>%
  group_by(SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_ID, PROV_NAME_WID, PROV_TYPE, ACTIVE_STATUS, Appt.Made.YearMonth, waitTime_14days) %>%
  summarise(total = n()) %>%
  collect() %>%
  pivot_wider(names_from = waitTime_14days,
              values_from = total,
              values_fill = 0) %>%
  mutate(total_new_scheduled = (Yes+No),
         perc_14days = Yes/(Yes+No)) %>%
  mutate(MSHS = "MSHS") %>%
  mutate(Provider = trimws(gsub("\\[.*?\\]", "", PROV_NAME_WID)))
```
...
reactable(
  ...
  #           }
  #         },
  groupBy = c("MSHS", "Site", "Department", "Specialty", "PROV_TYPE", "Provider", "ACTIVE_STATUS", "Slot.YearMonth"),
  columns = list(
    Site = colDef(
      name = "Site",
      minWidth = 180),
    ...
    NEW_BLOCK_TIME_RELEASE_PERC = colDef(
      name = "% of New Patient Blocks Converted to 7 or 14-Day Late-Releasing Blocks",
      maxWidth = 150,
     headerStyle = list(background = "#a5a7a5", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalTOTAL_NEW_BLOCKS = 0
     ...
     style = JS("function(rowInfo) {
        var value = rowInfo.row['NEW_BLOCK_TIME_RELEASE_PERC']
        if (value >= 0.75) {
          var color = '#008000'
        } else if (value < 0.75 ) {
          var color = '#e00000'
...
reactable(
  ...
  columns = list(
    ...
    NEW_BLOCK_TIME_RELEASE_PERC = colDef(
      name = "% of New Patient Blocks Converted to 7/14-Day Late-Releasing Blocks",
      maxWidth = 150,
     headerStyle = list(background = "#a5a7a5", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalTOTAL_NEW_BLOCKS = 0
...
## New Patients Scheduled within 14 Days (Physicians Only)
```{r New Patients Scheduled within 14 Days (Physicians) Output, echo = FALSE, warning = FALSE, message = FALSE}
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('new-pt-waitTime-physician', 'New Patient Wait Time (Physicians Only)')"
    ),
    
reactable(
  new_pt_waitTime %>%
    filter(PROV_TYPE == "Physician") %>%
    dplyr::select(MSHS, SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, DEPARTMENT_ID, PROV_TYPE, PROV_ID, PROV_NAME_WID, Provider, Appt.Made.YearMonth, total_new_scheduled, Yes, No, perc_14days),
  elementId = "new-pt-waitTime",
  ...
  groupBy = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "Provider", "Appt.Made.YearMonth"),

  columnGroups = list(
    colGroup(name = "", columns = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "PROV_TYPE", "Provider", "Appt.Made.YearMonth"),
             headerStyle = list(color = "black", fontSize = "15px"),
             sticky = "left"),
    colGroup(name = paste0("New Patients Scheduled ", waitTime_date_start, " - ",  as.Date(waitTime_date_end)-days(1)), 
             columns = c("total_new_scheduled", "Yes", "No", "perc_14days"),
  ...
  columns = list(
    ...
    Appt.Made.YearMonth = colDef(
      name = "Year-Month",
      minWidth = 100),

    total_new_scheduled = colDef(
      name = "New Patients Scheduled",
      maxWidth = 100,
    ...
    perc_14days = colDef(
      ...
      style = JS("function(rowInfo) {
        var value = rowInfo.row['perc_14days']
        if (value >= 0.6) {
          var color = '#008000'
        } else if (value < 0.6 ) {
          var color = '#e00000'
    ...
    DEPARTMENT_ID = colDef(show =F),
    PROV_ID = colDef(show =F),
    PROV_NAME_WID = colDef(show =F),
    ACTIVE_STATUS = colDef(show =F)
