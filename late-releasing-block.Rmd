...
```{r Block Usage (Physicians), echo = FALSE, warning = FALSE, message = FALSE}
# Table Output -----------------------------------------------------------------
htmltools::browsable(
  tagList(
    tags$button(
...
## Block Distribution (All Providers)
```{r Block Usage (All), echo = FALSE, warning = FALSE, message = FALSE}
# Table Output -----------------------------------------------------------------
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Table"),
      onclick = "Reactable.downloadDataCSV('block-distribution', 'Template Block Distribution')"
    ),
# htmltools::browsable(
#   tagList(
#     div(tags$label("Group by", `for` = "cars-grouping-select")),
...
reactable(
  block_error %>%
    arrange(MSHS, SITE, DEPT_SPECIALTY_NAME, DEPARTMENT_NAME, PROV_TYPE, PROV_NM_WID, slot_year, slot_month, SLOT_BEGIN_TIME) %>%
    filter(PROV_TYPE == "Physician") %>%
    mutate(SLOT_BEGIN_TIME = as.character(SLOT_BEGIN_TIME)),
  ...
  groupBy = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "slot_year", "slot_month", "PROV_NM_WID"),
  
  columnGroups = list(
    colGroup(name = "", columns = c("MSHS", "SITE", "DEPT_SPECIALTY_NAME", "DEPARTMENT_NAME", "slot_year", "slot_month", "PROV_NM_WID", "PROV_TYPE", "ACTIVE_STATUS",
                                    "SLOT_BEGIN_TIME", "FINAL_BLOCKS"),
             headerStyle = list(color = "black", fontSize = "15px"),
             sticky = "left"),
    colGroup(name = paste0("Total Openings"),
