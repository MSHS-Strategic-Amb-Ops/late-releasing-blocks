...
new_blocks <- c(
  ...
  "New Patient freeze", 
  "New Patient Only", 
  "New Patient pt, New Patient Start", 
  ...
  "COVID-19 SCREENING TELEHEALTH NEW PATIENT", 
  "Initial Visit", 
  "New Pt - Straight Medicare", 
  "NEW SELIKOFF", 
  "OPR New Patient", 
  "Remote Monitoring Initial", 
  "Telephone New Patient", 
  "Video Visit New Extended", 
  "Video Visit New Patient", 
...
late_block_raw$Campus.Specialty <- dept_mapping$DEPT_SPECIALTY_NAME[match(late_block_raw$DEPARTMENT_ID, dept_mapping$DEPARTMENT_ID)]
```
...
late_block_raw <- blocks_combined %>%
  ...
  group_by(Campus, Campus.Specialty, Department, DEPARTMENT_ID, Provider, Slot.YearMonth, SLOT_BEGIN_TIME, block_combination, new_block, release_block) %>%
  summarise(total = n()) 
  

# Summary of slot type by month   
new_block_conversion <- late_block_raw %>%
  ...
  group_by(Campus, Campus.Specialty, Department,  DEPARTMENT_ID, Provider, Slot.YearMonth) %>%
  ...
  rowwise() %>% 
  mutate(TOTAL_BLOCKS = sum(c_across(where(is.numeric)), na.rm = T)) %>%
...
order <- c("Campus","Campus.Specialty","Department","DEPARTMENT_ID","Provider","Slot.YearMonth",
...
reactable(
  ...
  columns = list(
    ...
    NEW_BLOCK_TIME_RELEASE = colDef(
      name = "Total New Patient Blocks Converted to Late-Releasing New Patient Blocks",
      maxWidth = 200,
      headerStyle = list(background = "#d80b8c", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = 'sum',
      format = colFormat(separators = TRUE, digits = 0),
    ...
    NEW_BLOCK_TIME_RELEASE_PERC = colDef(
      name = "% of New Patient Blocks Converted to Late-Releasing New Patient Blocks",
      maxWidth = 200,
     headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalTOTAL_NEW_BLOCKS = 0
    ...
    Campus.Specialty = colDef(show =F),
