...
block_combination_usage <- late_block_raw %>%
  ...
  mutate(Slot.Year = year(SLOT_BEGIN_TIME),
         Slot.Month = month(SLOT_BEGIN_TIME)) %>%
  group_by(Campus, DEPARTMENT_ID, Department, Provider, Slot.Year, Slot.Month, block_combination) %>%
  summarise(total = n()) %>%
  rename(Site = Campus,
         total_slots = total)
...
# write.xlsx(data_list, "Block Types and Usage_5.17.2023.xlsx")
