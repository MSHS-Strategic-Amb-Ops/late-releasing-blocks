...
#   collect()
# 
# access_neuro <- access_raw_tbl %>%
#   filter(DEPARTMENT_ID == 8811010) %>%
#   mutate(Appt.DateYear = TO_DATE(CONTACT_DATE)) %>%
#   filter(Appt.DateYear >= as.Date("2023-11-01")) %>%
#   collect()

```


```{r Block Openings Analysis, echo = FALSE, warning = FALSE, message = FALSE}

block_openigns <- block_raw_tbl %>%
  ...
  mutate(Slot.DateYear = TO_DATE(SLOT_BEGIN_TIME),
         Slot.Year = year(SLOT_BEGIN_TIME),
         Slot.Month = month(SLOT_BEGIN_TIME)) %>%
  filter(Slot.DateYear >= as.Date("2023-07-01")) %>%
  collect()

block_openigns <- block_openigns %>%
  left_join(prov_mapping) 
...
# slot_data <- availability_raw_tbl %>%
#   filter(DEPARTMENT_ID == 8827017) %>%
#   mutate(Slot.DateYear = TO_DATE(SLOT_BEGIN_TIME),
#          Slot.Year = year(SLOT_BEGIN_TIME),
#          Slot.Month = month(SLOT_BEGIN_TIME)) %>%
...
# 
# 
# late_block_raw_check <- late_block_raw %>%
#   filter(DEPARTMENT_ID == 8827017) %>%
#   collect()
# 
# # Block combinations by time slot ----------------------------------------------
# ## Shows all blocks associated to each time slot -------------------------------
# blocks_combined <- late_block_raw %>%
#   left_join(availability_raw_tbl %>% 
#               dplyr::select(DEPARTMENT_ID, PROV_ID, SLOT_BEGIN_TIME, ORG_REG_OPENINGS, ORG_OVBK_OPENINGS)) %>%
#     # For Test Run
#   filter(DEPARTMENT_ID == 8827017) %>%
#   mutate(Slot.DateYear = TO_DATE(SLOT_BEGIN_TIME),
#          Slot.Year = year(SLOT_BEGIN_TIME),
